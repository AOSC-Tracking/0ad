...
{
	if (std::queue<SDL_Event>& priorityEvents{m_Storage->m_PriorityEvents};
		!priorityEvents.empty())
	{
		*m_Storage->m_Event = priorityEvents.front();
		priorityEvents.pop();
	}
	else if (!SDL_PollEvent(&*m_Storage->m_Event))
		m_Storage = nullptr;
...
bool operator==(const Manager::PollEventsResult::EventIterator& lhs,
	const Manager::PollEventsResult::EventIterator& rhs)
{
