...
public:
	...
	const PS::vector<CStr>& GetEnabledMods() const;
	const PS::vector<CStr>& GetIncompatibleMods() const;
	const PS::vector<ModData>& GetAvailableMods() const;
	 ...
	 */
	bool EnableMods(const PS::vector<CStr>& mods, const bool addPublic);
	 ...
	 */
	const PS::vector<const Mod::ModData*> GetEnabledModsData() const;

	/**
	 * @return whether the two lists are compatible for replaying / MP play.
	 */
	static bool AreModsPlayCompatible(const PS::vector<const Mod::ModData*>& modsA, const PS::vector<const Mod::ModData*>& modsB);
private:

	/**
	 * Checks a list of @a mods and returns the incompatible mods, if any.
	 */
	PS::vector<CStr> CheckForIncompatibleMods(const PS::vector<CStr>& mods) const;
	bool CompareVersionStrings(const CStr& required, const CStr& op, const CStr& version) const;

	PS::vector<CStr> m_EnabledMods;
	// Of the currently loaded mods, these are the incompatible with the engine and cannot be loaded.
	PS::vector<CStr> m_IncompatibleMods;

	PS::vector<ModData> m_AvailableMods;
};
