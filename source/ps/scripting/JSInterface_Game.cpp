...
void SetPlayerID(const WithRequest& rq, int id)
{
	if (!g_Game)
		return;
...
void SetViewedPlayer(const WithRequest& rq, int id)
{
...
bool IsPaused(const WithRequest& rq)
{
	if (!g_Game)
	{
...
void SetPaused(const WithRequest& rq, bool pause, bool sendMessage)
{
	if (!g_Game)
	{
		ScriptException::Raise(rq, "Game is not started");
