...
void RegisterScriptFunctions_Settings(const WithRequest& rq)
{
...
JS::Value GetCameraRotation(const WithRequest& rq)
{
	if (!g_Game || !g_Game->GetView())
...
JS::Value GetCameraPivot(const WithRequest& rq)
{
	if (!g_Game || !g_Game->GetView())
		return JS::UndefinedValue();
...
JS::Value GetCameraPosition(const WithRequest& rq)
{
	if (!g_Game || !g_Game->GetView())
		return JS::UndefinedValue();
