...
void StartNetworkHost(const WithRequest& rq, const CStrW& playerName, const u16 serverPort, bool useSTUN,
...
void StartNetworkJoin(const WithRequest& rq, const CStrW& playerName, const CStr& serverAddress, u16 serverPort, bool storeReplay)
{
	ENSURE(!g_NetClient);
	ENSURE(!g_NetServer);
	ENSURE(!g_Game);
...
void SendNetworkChat(const WithRequest& rq, const CStrW& message, JS::HandleValue handle)
{
	ENSURE(g_NetClient);
