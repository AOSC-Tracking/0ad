...
public:
	 ...
	 */
	bool FromJSVal(const WithRequest& rq, JS::HandleValue value, const bool sendMessage);

	/**
	 * Converts the setting data to a JS::Value using Script::ToJSVal.
	 */
	virtual void ToJSVal(const WithRequest& rq, JS::MutableHandleValue value) = 0;
...
protected:
	...
	virtual bool DoFromJSVal(const WithRequest& rq, JS::HandleValue value) = 0;
