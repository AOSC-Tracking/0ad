...
bool ScriptException::IsPending(const WithRequest& rq)
{
	return JS_IsExceptionPending(rq.cx());
}

bool ScriptException::CatchPending(const WithRequest& rq)
{
...
void ScriptException::Raise(const WithRequest& rq, const char* format, ...)
{
	va_list ap;
	va_start(ap, format);
