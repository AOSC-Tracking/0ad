 ...
 */
std::string ToString(const WithRequest& rq, JS::MutableHandleValue obj, bool pretty = false);
 ...
 */
bool ParseJSON(const WithRequest& rq, const std::string& string_utf8, JS::MutableHandleValue out);

/**
 * Read a JSON file. Returns the unmodified value on error and prints an error message.
 */
void ReadJSONFile(const WithRequest& rq, const VfsPath& path, JS::MutableHandleValue out);

/**
 * Stringify to a JSON string, UTF-8 encoded. Returns an empty string on error.
 */
std::string StringifyJSON(const WithRequest& rq, JS::MutableHandleValue obj, bool indent = true);
}
