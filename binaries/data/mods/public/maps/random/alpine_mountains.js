...
export function* generateMap(mapSettings)
{
	...
	globalThis.g_Map = new RandomMap(heightLand, tGrassA);
	...
	while(true)
	{
		...
		const scaleArray = [0.3, 0.5, 0.8, 1, 1, 1, 1];
		const scale = scaleArray[scaleByMapSize(0, 6)];
		for (let x = 0; x < mapSize; ++x)
		{
			for (let y = 0; y < mapSize; ++y)
			{
				const value = noise.get(x * scale / (mapSize + 1.0), y * scale / (mapSize + 1.0)) - 0.54;
		...
		for (let x = 0; x < mapSize; ++x)
		{
			for (let y = 0; y < mapSize; ++y)
			{
				const value = noise.get(x * scale / (mapSize + 1.0), y * scale / (mapSize + 1.0)) - 0.565;
				const position = new Vector2D(x, y);
		...
		for (let x = 0; x < mapSize; ++x)
		{
			for (let y = 0; y < mapSize; ++y)
			{
				const value = noise.get(x * scale / (mapSize + 1.0), y * scale / (mapSize + 1.0));
				const position = new Vector2D(x, y);
		...
		if (!mapSettings.Nomad)
		{
			...
			const players = playerPlacementRandomScaled(
		...
		for (const forestVariant of forestVariants)
			createAreasInAreas(
				...
				scaleByMapAreaAndPlayers(2, !mapSettings.Nomad),
				0.5,
				[mountainBorderArea]);
		...
		if (mapSettings.Nomad)
		{
			...
			const initialConstraint = avoidClasses(clMountain, 3, clForest, 2);
			const floodFillTriedTiles = [];
			while (floodFillTriedTiles.length < 20)
			{
				const position = g_Map.randomCoordinate();
				if (initialConstraint.allows(position))
					floodFillTriedTiles.push(position);
			}

			let largestAreaSize = 0;
			for (const position of floodFillTriedTiles)
			{
				const connectedArea = floodFill(position, [avoidClasses(clMountain, 5, clForest, 0), stayClasses(clNonBordering, 0)]);
			...
			const nomadAreaFraction = largestAreaSize / diskArea(mapSize / 2);
		...
		else
		{
			...
			const connectedArea = floodFill(pickRandom(playerPosition), [avoidClasses(clMountain, 5, clForest, 0), stayClasses(clNonBordering, 0)]);
	...
	{
		const playerFactor = countPlayers ? getNumPlayers() : 0;
		return base * ((diskArea(g_Map.getSize()/2) - playerFactor * diskArea(20)) / (diskArea(minMapSize/2)));
	}
	...
	function playerPlacementRandomScaled(constraints = undefined)
	{
		let locations = [];
		let attempts = 0;
		let resets = 0;

		const minDistArray = [1, 1, 0.55, 0.35, 0.3, 0.25, 0.25, 0.25, 0.25];
		const playerMinDistFraction = minDistArray[getNumPlayers()];
