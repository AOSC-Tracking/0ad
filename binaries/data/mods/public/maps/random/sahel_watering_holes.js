...
function* GenerateMap()
{
	...
	const pForest = [
		tForestFloor + TERRAIN_SEPARATOR + oBaobab,
		tForestFloor + TERRAIN_SEPARATOR + oBaobab,
		tForestFloor
	];
	...
	const riverStart = distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers,
		fractionToTiles(0.15), mapCenter)[0];
	const riverEnd = distributePointsOnCircle(numPlayers, startAngle + Math.PI / numPlayers,
	...
	for (let i = 0; i < numPlayers; ++i)
	{
		...
		createArea(
			new PathPlacer(riverStart[i], riverEnd[i], scaleByMapSize(10, 50), 0.2,
				3 * scaleByMapSize(1, 4), 0.2, 0.05),
			[
		...
		// Animals in shallows
		const shallowPosition =
			Vector2D.average([playerPosition[i], playerPosition[neighborID]]).round();
		const objects = [
	...
	const [forestTrees, stragglerTrees] = getTreeCounts(160, 900, 0.02);
	const types = [
		[[tForestFloor, tGrass, pForest], [tForestFloor, pForest]]
	];

	const forestSize = forestTrees / (0.5 * scaleByMapSize(2, 8) * numPlayers);
		...
		createAreas(
			...
			[
				new LayeredPainter(
					[
						[tGrass, tDirtRocksA],
						[tDirtRocksA, tDirtRocksB],
						[tDirtRocksB, tDirtRocksC]
					],
					[1, 1]),
	...
	placePlayersNomad(clPlayer,
		avoidClasses(clWater, 4, clHill, 4, clForest, 1, clMetal, 4, clRock, 4, clFood, 2));
	...
	// greyish
	setWaterColor(0.478, 0.42, 0.384);
	// reddish
	setWaterTint(0.58, 0.22, 0.067);
