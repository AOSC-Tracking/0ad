...
g_SelectionPanels.Command = {
	...
	"setupButton": function(data)
	{
		...
		data.button.size.rleft = 50;
		data.button.size.rright = 50;
		// offset from the center calculation, count on square buttons, so size.bottom is the width too
		data.button.size.left = (data.i - data.numberOfItems / 2) * (data.button.size.bottom + 1);
		data.button.size.right = data.button.size.left + data.button.size.bottom;
