...
Treasure.prototype.ComputeReward = function () {
  for (let resource in this.template.Resources) {
    let amount = ApplyValueModificationsToEntity(
      "Treasure/Resources/" + resource,
...
Treasure.prototype.Reward = function (entity) {
  ...
  if (!cmpPlayer) return false;

  let cmpFogging = Engine.QueryInterface(this.entity, IID_Fogging);
  if (cmpFogging) cmpFogging.Activate();

  if (this.resources) cmpPlayer.AddResources(this.resources);
