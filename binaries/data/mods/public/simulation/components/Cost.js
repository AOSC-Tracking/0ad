...
Cost.prototype.GetResourceCosts = function () {
  ...
  if (!cmpOwnership) {
    error(
      "GetResourceCosts called without valid ownership on " + this.entity + "."
    );
    return {};
  }
  ...
  let entityTemplateName = cmpTemplateManager.GetCurrentTemplateName(
    this.entity
  );
...
Cost.prototype.OnValueModification = function (msg) {
  ...
  if (!cmpPlayer) return;
  if (popCostDifference) cmpPlayer.AddPopulation(popCostDifference);
};
